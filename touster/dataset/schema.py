...
class Sample:
    ...
    id: str = ""  # traceability — auto-assigned at save time if left blank
...
class Dataset:
    ...
    def to_list(self) -> list[dict]:
        """Serialize to list of {id, messages: [{role, content}, ...]} dicts."""
        return [
            {
                "id": sample.id or f"sample_{i:04d}",
                "messages": [
...
def _parse_sample(raw: object) -> Sample:
    ...
    raw_id = raw.get("id", "")
    return Sample(messages=messages, id=raw_id if isinstance(raw_id, str) else "")


def try_parse_sample(raw: object) -> Sample | None:
    """Parse one sample dict against the golden {id?, messages: [...]} schema.
...
def from_list(data: list[dict]) -> Dataset:
    """
    Parse list of {id?, messages: [{role, content}]} dicts into Dataset.
...
def load_jsonl(path: Path) -> Dataset:
    """Load a .jsonl file where each line is a {id?, messages: [...]} JSON object."""
    path = Path(path)
    if not path.exists():
...
def save_jsonl(ds: Dataset, path: Path) -> None:
    """Save a Dataset as .jsonl, one JSON object per line (id auto-assigned if blank)."""
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    # newline="" + explicit \n suffix ensures LF-only output on all platforms
    with path.open("w", encoding="utf-8", newline="") as fh:
        for obj in ds.to_list():
