...
def generate_dataset(
    ...
    Generates in batches. Returns Dataset.
    """
    if num_samples < 1:
    ...
    while len(good_samples) < num_samples:
        ...
        try:
            ...
            display.warning(f"Batch generation failed, skipping: {exc}")
            raw_batch = []
        ...
        if dropped:
            display.warning(f"Dropped {dropped} malformed sample(s) from batch (kept {len(valid)}).")
        ...
        print(f"Generating dataset... {len(good_samples)}/{num_samples}")
